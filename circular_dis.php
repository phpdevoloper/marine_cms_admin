...
if($request == 1){
 ## Read value
   ...
   $columnIndex = $_POST['order'][0]['column']; // Column index
    $columnName = "release_dt"; // Column name
    ...
    $sel = pg_query($connection,"SELECT count(*) as allcount from circular_mst");
    $records = pg_fetch_array($sel);
    $totalRecords = $records['allcount'];

    ## Total number of records with filtering
    $sel = pg_query($connection,"select count(*) as allcount from circular_mst WHERE ".$searchQuery);
    $records = pg_fetch_array($sel);
    $totalRecordwithFilter = $records['allcount'];

    ## Fetch records
    $sql = "select * from circular_mst WHERE ".$searchQuery." order by ".$columnName."  limit ".$rowperpage." OFFSET ".$rows;
...
$updateButton = "<i class='fa fa-edit updateUser' data-id='".$row['cir_id']."' data-toggle='modal' data-target='#updateModal' ></i>";
        // Delete Button
        $deleteButton = "<i class='fa fa-trash btn-danger deleteUser' data-id='".$row['cir_id']."'></i>";
...
$id = 0;
	...
	$sql = "SELECT * From circular_mst where cir_id=".$id;
	$query = pg_query($connection, $sql);
	$rowCount = pg_num_rows($query);
...
if($request == 3){
    ...
    // Check id
    $record = pg_query($connection,"SELECT * FROM circular_mst WHERE cir_id=".$id);
...
if($request == 5){
    echo $_FILES['txtupload_add']['name'];
		...
		echo $str="insert into circular_mst (type,category,title,release_dt,dt_upto,doc_path,doc_size,submittedby) 
		values('".$type."','".$category."','".$title."','".$release_dt."','".$validupto."','".$path_filename_ext."','".$file_size."','rama.a@nic.in')";
