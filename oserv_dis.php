...
if($request == 1){
 ## Read value
   ...
   $columnIndex = $_POST['order'][0]['column']; // Column index
    ...
    $sel = pg_query($connection,"SELECT count(*) as allcount from onlineservices_mst");
    $records = pg_fetch_array($sel);
    $totalRecords = $records['allcount'];

    ## Total number of records with filtering
    $sel = pg_query($connection,"select count(*) as allcount from onlineservices_mst WHERE ".$searchQuery);
    $records = pg_fetch_array($sel);
    $totalRecordwithFilter = $records['allcount'];

    ## Fetch records
    $sql = "select * from onlineservices_mst WHERE ".$searchQuery." order by ".$columnName."  limit ".$rowperpage." OFFSET ".$rows;
...
$updateButton = "<i class='fa fa-edit updateUser' data-id='".$row['oserv_id']."' data-toggle='modal' data-target='#updateModal' ></i>";
        // Delete Button
        $deleteButton = "<i class='fa fa-trash btn-danger deleteUser' data-id='".$row['oserv_id']."'></i>";
        
        $action = $updateButton." ".$deleteButton;

        $data[] = array(
                "oserv_id" => $row['oserv_id'],
                "title" => $row['title'],
                "type" => $row['type'],
...
$id = 0;
	...
	$sql = "SELECT * From onlineservices_mst where oserv_id=".$id;
	$query = pg_query($connection, $sql);
	$rowCount = pg_num_rows($query);
...
if($request == 3){
    ...
    // Check id
    $record = pg_query($connection,"SELECT * FROM onlineservices_mst WHERE oserv_id=".$id);
    if(pg_num_rows($record) > 0){
		...
		$str="update onlineservices_mst set title='".$title."',type='".$type."',category='".$category."',dt_upto='".$validupto."',release_dt='".$release_dt."',doc_path='".$path_filename_ext."',doc_size='".$sfile_size."' where news_id=".$id; 
...
if($request == 5){
    echo $_FILES['txtupload_add']['name'];
		...
		echo $str="insert into onlineservices_mst (type,category,title,release_dt,dt_upto,doc_path,submittedby,doc_size) 
