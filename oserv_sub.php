...
if(!empty($_POST["submit"]))
{
 $doc_type=$_POST['txtdoc'];
	...
	if($_FILES['txtupload']['name']!="")
		...
		{
		  $str="update onlineservices_mst set title='".$title."',type='".$type."',doc_type='".$doc_type."',doc_path='".$path_filename_ext."',doc_size='".$sfile_size."' where oserv_id=".$id; 
		...
		{
			header("Location: cms_onlineservices.php?C=F");
...
// var_dump($_POST); exit;
		if((chkalphanum_title($title) != '2') && (chklen1($type) != '2') &&  (chklen4($doc_type)!='2') )
		{
		  $str="update onlineservices_mst set title='".$title."',type='".$type."',doc_type='".$doc_type."',url_txt='".$url_txt."' where oserv_id=".$id; 
		//   var_dump($str);exit;die();
          pg_query($connection,$str);			
		  header("Location: cms_onlineservices.php?C=S");
          exit;
...
// echo 'HIIIIIII';exit;
		if((chkalphanum_title($title) != '2') && (chklen1($type) != '2') &&  (chklen4($doc_type)!='2') )
		{
			$str="insert into onlineservices_mst (type, title, doc_type, url_txt,submittedby) values('".$type."','".$title."','".$doc_type."','".$url_txt."','rama.a@nic.in')";
			// var_dump($str);exit;die();
			pg_query($connection,$str);			
		  header("Location: cms_onlineservices.php?C=S");
          exit;
