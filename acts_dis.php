...
if($request == 1){
 ## Read value
    ...
    $sel = pg_query($connection,"SELECT count(*) as allcount from acts_mst");
    $records = pg_fetch_array($sel);
    $totalRecords = $records['allcount'];

    ## Total number of records with filtering
    $sel = pg_query($connection,"select count(*) as allcount from acts_mst WHERE ".$searchQuery);
    $records = pg_fetch_array($sel);
    $totalRecordwithFilter = $records['allcount'];

    ## Fetch records
    $sql = "select * from acts_mst WHERE ".$searchQuery." order by ".$columnName."  limit ".$rowperpage." OFFSET ".$rows;
   // $empRecords = pg_query($connection, $empQuery);
    $data = array();
   

    $sql = "SELECT * From acts_mst order by release_dt";
...
$updateButton = "<i class='fa fa-edit updateUser' data-id='".$row['act_id']."' data-toggle='modal' data-target='#updateModal' ></i>";
        // Delete Button
        $deleteButton = "<i class='fa fa-trash btn-danger deleteUser' data-id='".$row['act_id']."'></i>";
...
if($request == 2){
	...
	$sql = "SELECT * From acts_mst where act_id=".$id;
	$query = pg_query($connection, $sql);
	$rowCount = pg_num_rows($query);
...
if($request == 3){
    ...
    // Check id
    $record = pg_query($connection,"SELECT * FROM acts_mst WHERE act_id=".$id);
    if(pg_num_rows($record) > 0){
		...
		$str="update acts_mst set title='".$title."',type='".$type."',category='".$category."',dt_upto='".$validupto."',release_dt='".$release_dt."',doc_path='".$path_filename_ext."',doc_size='".$sfile_size."' where act_id=".$id; 
...
if($request == 5){
    echo $_FILES['txtupload_add']['name'];
		...
		echo $str="insert into acts_mst (type,category,title,release_dt,dt_upto,doc_path,submittedby,doc_size) 
