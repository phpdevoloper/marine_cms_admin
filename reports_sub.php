...
if(!empty($_POST["submit"]))
{
 $doc_type=$_POST['txtdoc'];
	...
	if($_FILES['txtupload']['name']!="")
		...
		{
		echo  $str="update reports_mst set title='".$title."',type='".$type."',doc_type='".$doc_type."',doc_path='".$path_filename_ext."',doc_size='".$sfile_size."' where rep_id=".$id; 
          pg_query($connection,$str);			
		  header("Location: cms_reports.php?C=S");
          exit;
		}
		else
		{
			header("Location: cms_reports.php?C=F");
...
// var_dump($_POST); exit;
		if((chkalphanum_title($title) != '2') && (chklen1($type) != '2') &&  (chklen4($doc_type)!='2') )
		{
		  $str="update reports_mst set title='".$title."',type='".$type."',doc_type='".$doc_type."',url_txt='".$url_txt."' where rep_id=".$id; 
...
// echo "innnn";
		...
		//if((chkalphanum_title($title) != '2') && (chklen1($type) != '2') &&  (chklen4($doc_type)!='2') )
		//{
			echo $str="insert into reports_mst (type, title, doc_type, doc_path, submittedby, doc_size) values('".$type."','".$title."','".$doc_type."','".$path_filename_ext."','rama.a@nic.in','".$sfile_size."')";
			// var_dump($str);exit;die();
          pg_query($connection,$str);			
		 header("Location: cms_reports.php?C=S");
//          exit;
...
// echo 'HIIIIIII';exit;
		if((chkalphanum_title($title) != '2') && (chklen1($type) != '2') &&  (chklen4($doc_type)!='2') )
		{
			$str="insert into reports_mst (type, title, doc_type, url_txt,submittedby) values('".$type."','".$title."','".$doc_type."','".$url_txt."','rama.a@nic.in')";
			// var_dump($str);exit;die();
			pg_query($connection,$str);			
		  header("Location: cms_reports.php?C=S");
          exit;
