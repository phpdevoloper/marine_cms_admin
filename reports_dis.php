...
if($request == 1){
 ## Read value
   ...
   $columnIndex = $_POST['order'][0]['column']; // Column index
    ...
    $sel = pg_query($connection,"SELECT count(*) as allcount from reports_mst");
    $records = pg_fetch_array($sel);
    $totalRecords = $records['allcount'];

    ## Total number of records with filtering
    $sel = pg_query($connection,"select count(*) as allcount from reports_mst WHERE ".$searchQuery);
    $records = pg_fetch_array($sel);
    $totalRecordwithFilter = $records['allcount'];

    ## Fetch records
   $sql = "select * from reports_mst WHERE ".$searchQuery." order by ".$columnName."  limit ".$rowperpage." OFFSET ".$rows;
...
$updateButton = "<i class='fa fa-edit updateUser' data-id='".$row['rep_id']."' data-toggle='modal' data-target='#updateModal' ></i>";
        // Delete Button
        $deleteButton = "<i class='fa fa-trash btn-danger deleteUser' data-id='".$row['rep_id']."'></i>";
...
$id = 0;
	...
	$sql = "SELECT * From reports_mst where rep_id=".$id;
	$query = pg_query($connection, $sql);
	$rowCount = pg_num_rows($query);
...
if($request == 3){
    ...
    // Check id
    $record = pg_query($connection,"SELECT * FROM reports_mst WHERE rep_id=".$id);
    if(pg_num_rows($record) > 0){
		...
		$str="update reports_mst set title='".$title."',type='".$type."',category='".$category."',dt_upto='".$validupto."',release_dt='".$release_dt."',doc_path='".$path_filename_ext."',doc_size='".$sfile_size."' where rep_id=".$id; 
...
if($request == 5){
    echo $_FILES['txtupload_add']['name'];
		...
		echo $str="insert into reports_mst (type,category,title,release_dt,dt_upto,doc_path,submittedby,doc_size) 
